...
public class ExportPlaylistCommand extends AbstractCommand {
    ...
    public boolean execute() throws AudioLibraryCheckedException {
        ...
        for (Object id : songIds) {
            ArrayList<Object> songFields =
                    dbWrapper.selectCheckIfExists(new Song((int) id, "", "", 0), forCheck, forGet);
            songs.add(
                    new Song(
                            (String) songFields.get(0),
                            (String) songFields.get(1),
                            (Integer) songFields.get(2)));
        }

        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyyMMdd");
        String fileName =
                "src/main/java/org/example/ExportOutput/export_"
