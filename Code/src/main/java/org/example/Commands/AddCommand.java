    ...
    public boolean execute() {
        ...
        for(int i=2; i<getArgs().length; i++){
            try{
                ...
                PlaylistSongs playlistSongs = new PlaylistSongs(playlistId, Integer.parseInt(getArgs()[i]));

                if(!dbWrapper.selectCheckIfExists(playlistSongs, forCheckBefore, forGetBefore).isEmpty()){
                    existError = true;
                    throw new DataAlreadyAddedException("The song \"" + songName + "\" by \"" + songAuthor + "\" is already part of \"" + playlistName + "\"");
                }

                songs.add(new Song(songName, songAuthor, 0));
                playlistWithSongs.add(new PlaylistSongs(playlistId, Integer.parseInt(getArgs()[i])));
        ...
        for(int i=0; i<getArgs().length-2; i++){
            ...
            successMessage.append("Added \"").append(songs.get(i).getName()).append("\" by ").append(songs.get(i).getAuthor()).append(" to ").append(playlistName).append(".\n");
        }
